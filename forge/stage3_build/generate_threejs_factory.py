    ...
    if "silhouette-inflation" in used_primitives:
        lines.extend(
            [
                ...
                "  hand?: 'left' | 'right';",
                "  // Provenance the builder does not read but the descriptor carries, so the emitted",
                "  // literal typechecks against the shape stage 3 actually writes.",
                "  projection?: 'orthographic';",
                "  boundsSpace?: 'component-local';",
                "  hands?: string[];",
                "  inflation?: string;",
                "  sourceViewIds?: string[];",
                "  depthAxis?: { state: 'observed' | 'inferred'; source: string };",
                "}",
                "",
                ...
                "  const occupied = (mask: string[], row: number, col: number): boolean => (",
                "    row >= 0 && col >= 0 && row < size && col < size && mask[row][col] === '1'",
                "  );",
                "  const inside = (row: number, col: number): boolean => occupied(descriptor.mask, row, col);",
                "  const chamfer = (mask: string[]): { field: number[][]; peak: number } => {",
                "    const field: number[][] = [];",
                ...
                "  const halfThickness = (row: number, col: number, isFront: boolean): number => {",
                ...
                "  // A rim vertex duplicates a surface position so it can carry the uv of its own side.",
                "  // The position is identical, so a position-welded edge count still sees a closed surface.",
                "  const vertex = (row: number, col: number, isFront: boolean, rim = false): number => {",
