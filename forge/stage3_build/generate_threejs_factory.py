...
def geometry_for(
    ...
    if primitive == "silhouette-inflation":
        # The glove shell arrives as the parameters it was measured from, not as triangles, so the
        # runtime rebuilds it in code and the grid becomes an LOD knob.
        inflation = descriptor.get("silhouetteInflation")
        if not isinstance(inflation, dict):
            raise ValueError("silhouette-inflation requires geometryDescriptor.silhouetteInflation")
        return f"buildSilhouetteInflationGeometry({json_literal(inflation)})"
    if primitive == "tube":
...
def generate(spec: dict[str, Any], pass_id: str) -> str:
    ...
    if "silhouette-inflation" in used_primitives:
        lines.extend(
            [
                "interface SilhouetteInflationDescriptor {",
                "  grid: number;",
                "  mask: string[];",
                "  palmThicknessRatio: number;",
                "  aspect: number;",
                "  handSeparation: number;",
                "  hand?: 'left' | 'right';",
                "}",
                "",
                "// Rebuilds the shell the intake measured: a chamfer distance to the silhouette edge",
                "// drives a medial-axis inflation, and the boundary is sealed by one rim quad per cell",
                "// edge that a single solid cell owns. Ported from forge/stage3_build/glove_shell.py;",
                "// the parity test pins the two implementations to the same vertices.",
                "function buildSilhouetteInflationGeometry(descriptor: SilhouetteInflationDescriptor): THREE.BufferGeometry {",
                ...
                "    row >= 0 && col >= 0 && row < size && col < size && descriptor.mask[row][col] === '1'",
                "  );",
                "  const distance: number[][] = [];",
                "  for (let row = 0; row < size; row += 1) {",
                "    distance.push(new Array(size).fill(0));",
                "    for (let col = 0; col < size; col += 1) distance[row][col] = inside(row, col) ? size * size : 0;",
                "  }",
                "  for (let row = 0; row < size; row += 1) {",
                "    for (let col = 0; col < size; col += 1) {",
                "      if (!distance[row][col]) continue;",
                "      let best = distance[row][col];",
                "      if (row > 0) best = Math.min(best, distance[row - 1][col] + 1);",
                "      if (col > 0) best = Math.min(best, distance[row][col - 1] + 1);",
                ...
                "      if (!distance[row][col]) continue;",
                "      let best = distance[row][col];",
                "      if (row < size - 1) best = Math.min(best, distance[row + 1][col] + 1);",
                "      if (col < size - 1) best = Math.min(best, distance[row][col + 1] + 1);",
                "      distance[row][col] = best;",
                "    }",
                "  }",
                "  let peak = 0;",
                "  for (const row of distance) for (const value of row) peak = Math.max(peak, value);",
                "  peak = Math.max(1, peak);",
                "  const palmThickness = descriptor.palmThicknessRatio * descriptor.aspect;",
                "  const thickness = (row: number, col: number): number => (",
                "    palmThickness * Math.sqrt(Math.min(1, distance[row][col] / peak))",
                "  );",
                ...
                "  const vertex = (row: number, col: number, front: boolean): number => {",
                "    const key = `${row},${col},${front ? 1 : 0}`;",
                ...
                "    const half = thickness(row, col) / 2;",
                "    positions.push(hand === 'right' ? -x + offset : x + offset, y, front ? half : -half);",
                "    uvs.push((col + 0.5) / size, 1 - (row + 0.5) / size);",
                "    return id;",
                "  };",
                "  const outwardFront = hand === 'right';",
                "  const cells = Array.from(solid).map((key) => key.split(',').map(Number) as [number, number]);",
                ...
                "      const [a, b, c, d] = corners.map(([r, cc]) => vertex(r, cc, front));",
                "      if (front === outwardFront) indices.push(a, b, c, a, c, d);",
                "      else indices.push(a, c, b, a, d, c);",
                "    }",
                "    const rim: [[number, number], [number, number], [number, number]][] = [",
