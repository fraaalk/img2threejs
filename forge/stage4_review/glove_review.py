"""Fail-closed v2 evaluator for evidence-backed glove artifacts."""
...
REQUIRED_METRICS = {
    "evidenceCoverage": ("min", 0.95), "graphCompleteness": ("min", 0.95),
    "seamBoundaryCorrespondence": ("min", 0.95), "attachmentContinuity": ("min", 0.95),
    "finiteGeometry": ("min", 0.95), "selfIntersectionFree": ("min", 0.95),
    "penetrationFree": ("min", 0.95), "productionManifold": ("min", 0.95),
    "crossSectionVariation": ("min", 0.85), "handednessCorrect": ("min", 0.95),
    "asymmetryPreserved": ("min", 0.95), "uvMaterialOwnership": ("min", 0.95),
    "runtimeDeterministic": ("min", 0.95),
}
...
def build_calibrated_scene() -> dict[str, Any]:
    """Expose legacy values for diagnostics only; it cannot authorize v2 readiness."""
    scene = build_uncalibrated_scene()
    scene["thresholdStatus"] = "diagnostic-frozen-v1"
    scene["thresholds"] = {key: value[1] for key, value in REQUIRED_METRICS.items()}
    scene["thresholdSource"] = "diagnostic-only: forge/tests/fixtures/glove_review_measurements_v1.json"
    scene["calibration"] = {"eligibleForReadiness": False, "reason": "synthetic v1 fixture has no verified evidence-backed artifact chain"}
    return scene
...
def validate_glove_scene(scene: Any, *, calibrated: bool | None = None) -> list[str]:
    ...
    if calibrated is True:
        ...
        if not set(REQUIRED_METRICS).issubset(set(scene.get("thresholds", {}))):
            errors.append("calibrated scene thresholds are incomplete")
    return errors
...
def _derived_metrics(report: dict[str, Any], bundle: dict[str, Any], capture: dict[str, Any], meshes: list[dict[str, Any]], provenance_verified: bool, production_valid: bool) -> dict[str, dict[str, Any]]:
    ...
    derivation_ok = provenance_verified and topology_ready
    values = {
        "evidenceCoverage": float(derivation_ok),
        "graphCompleteness": float(topology_ready),
        "seamBoundaryCorrespondence": float(topology_ready),
        "attachmentContinuity": float(topology_ready),
        ...
        "productionManifold": float(topology_ready),
        "crossSectionVariation": float(all(mesh.get("measurements", {}).get("minimumThickness", 0.0) >= 0.01 for mesh in meshes if isinstance(mesh, dict))),
        "handednessCorrect": float(topology_ready and normals_valid),
        "asymmetryPreserved": float(topology_ready and any(mesh.get("hand") == "left" for mesh in meshes) and any(mesh.get("hand") == "right" for mesh in meshes)),
    ...
    sources = {"geometryReportDigest": report.get("reportDigest"), "modelBundleDigest": bundle.get("rootDigest"), "captureManifestDigest": capture.get("manifestDigest")}
    return {key: {"value": value, "status": "measured" if value in {0.0, 1.0} else "inconclusive", "unit": "ratio", "source": sources, "computation": "glove-review-v2-artifact-derived"} for key, value in values.items()}
...
def _verify_bundle_provenance(bundle: dict[str, Any], bundle_path: Path, manifest: dict[str, Any]) -> bool:
...
def evaluate_glove_review(manifest: dict[str, Any], artifacts: dict[str, Any], scene: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        provenance_verified = _verify_bundle_provenance(bundle, bundle_path, manifest)
        ...
        bundle, report, capture, meshes, production_errors, provenance_verified, capture_failures = {}, {}, {}, [], ["artifact verification failed"], False, [f"artifact-chain:{error}"]
    failed.extend(capture_failures)
    metrics = _derived_metrics(report, bundle, capture, meshes, provenance_verified, not production_errors) if bundle and report else {}
    for key, (_direction, threshold) in REQUIRED_METRICS.items():
        metric = metrics.get(key)
        if not metric or metric["status"] != "measured":
            failed.append(f"metric-inconclusive:{key}")
    ...
    output = {"version": "glove-review-report.v2", "verdict": "ready" if not failed else "reject", "action": action, "family": manifest.get("itemFamily"), "subtype": manifest.get("subtype"), "modelBundleDigest": bundle.get("rootDigest"), "scene": {"version": scene.get("version"), "fixtureId": scene.get("fixtureId"), "thresholdStatus": scene.get("thresholdStatus")}, "metrics": metrics, "failedGates": sorted(set(failed)), "evidenceTier": report.get("evidenceTier")}
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Evaluate verified CS2 Sport Gloves v2 artifacts")
    ...
    try:
        report = evaluate_glove_review(_load_json(args.manifest, "manifest"), _load_json(args.artifacts, "artifacts"), _load_json(args.scene, "scene"))
        _write_atomic(args.out, report); print(json.dumps(report, indent=2, ensure_ascii=False)); return 0 if report["verdict"] == "ready" else 1
    except (OSError, ValueError, json.JSONDecodeError) as error:
